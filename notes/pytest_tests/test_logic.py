...
'''Не работает это в Dajngo 5.1. Пришлось переписывать'''
...
def test_not_unique_slug(author_client, note, form_data):
    url = reverse('notes:add')
    form_data['slug'] = note.slug  # Используем существующий slug
    response = author_client.post(url, data=form_data)

    # Проверяем, что форма содержит ошибку
    assert response.context['form'].errors == {
        'slug': [note.slug + WARNING]  # Ожидаемая структура ошибки
    }
    assert Note.objects.count() == 1  # Новая заметка не создана
